...
const Dashboard = () => {
  ...
  const fetchConversations = async () => {
	...
	const res = await fetch(`https://talkme-3backend.onrender.com/api/conversations/${loggedInUser?.id}`, {
	  method: 'GET',
	  headers: {
  ...
  useEffect(() => {
	const fetchUsers = async () => {
	  const res = await fetch(`https://talkme-3backend.onrender.com/api/users/${user?.id}`, {
		method: 'GET',
		headers: {
		  'Content-Type': 'application/json',
		}
  ...
  const fetchMessages = async (conversationId, receiver) => {
	...
	if (conversationId === 'new') {
	  const createConvoRes = await fetch(`https://talkme-3backend.onrender.com/api/conversations`, {
		method: 'POST',
		headers: {
		  'Content-Type': 'application/json',
		},
	...
	const res = await fetch(`https://talkme-3backend.onrender.com/api/message/${id}?senderId=${user?.id}&receiverId=${receiver?.receiverId}`, {
	  method: 'GET',
	  headers: {
		'Content-Type': 'application/json',
	  }
  ...
  const sendMessage = async () => {
	...
	await fetch(`https://talkme-3backend.onrender.com/api/message`, {
	  method: 'POST',
	  headers: {
		'Content-Type': 'application/json',
	  },
  ...
  return (
    <div className="w-screen flex relative">
      {/* Left Section */}
      <div className="w-[25%] h-screen bg-secondary border-r-2 border-main ">
      ...
      {/* Middle Section */}
      <div className="w-[75%] h-screen bg-blue-300 flex flex-col items-center ">
    {
        ...
        <div className="p-20">
            {
                messages?.messages?.length > 0 ?
                    messages.messages.map(({ message, user: { id } = {} }, index) => (
                        <div className={`max-w-[45%] font-normal rounded-b-xl p-4 mb-6 text-white ${id === user?.id ? 'ml-auto bg-blacky text-white rounded-tl-xl' : 'mr-auto bg-light rounded-tr-xl text-primary'}`} key={index}>
                            {message}
                        </div>
                    )) : <div className="text-center text-white font-semibold text-lg mt-[90%]">No Messages</div>
            }
            <div ref={messageRef} />
        </div>
    </div>
    {
      ...
      <div className={`w-[2%] h-screen bg-light px-8 py-16 flex flex-col items-center justify-center border-l-2 border-main`}>
        <button className="toggle-friends-button" onClick={() => setShowFriends(!showFriends)}>
          ...
          <div className="text-white font-normal">{showFriends ? 'Close' : 'Friend List'}</div>
        </button>
      </div>

      {/* Friend List Section */}
      <div className={`w-[25%] h-screen bg-light px-8 py-16  fixed top-0 right-0 sidebar ${showFriends ? 'open' : ''}`}>
        ...
        <div className='overflow-scroll h-[625px]'>
          {
            users.length > 0 ?
